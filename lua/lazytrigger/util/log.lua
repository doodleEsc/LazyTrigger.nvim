...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.LazyTrigger.config ~= nil and not _G.LazyTrigger.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[lazytrigger.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "lazytrigger.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h LazyTrigger.options` to read more."
        )
    end
end
