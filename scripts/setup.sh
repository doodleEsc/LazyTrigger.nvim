...
rm -rf doc
mv plugin/lazytrigger.lua plugin/$PLUGIN_NAME.lua
mv lua/lazytrigger lua/$PLUGIN_NAME
...
grep -rl "LazyTrigger" .github/ plugin/ tests/ lua/ | xargs sed -i "" -e "s/YourPluginName/$PASCAL_CASE_PLUGIN_NAME/g"
grep -rl "lazytrigger" README.md .github/ plugin/ tests/ lua/ | xargs sed -i "" -e "s/lazytrigger/$PLUGIN_NAME/g"
